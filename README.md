<h1 align="center">¡Hola! Soy Cinthia 👋</h1>
<p align="center">
Diseñadora UI/UX · Automatización industrial · Visión por computadora (Python + PLC)
</p>

---

### 🧭 Sobre mí
- 🎯 Actualmente: inspección visual con **cámaras industriales + Python + PLC**.
- ✨ Me enfoco en **UI/UX**, **buenas prácticas** y **automatización confiable**.
- 📬 Contacto: **cinthia.parra.solorio@outlook.com** · LinkedIn: **https://www.linkedin.com/cinthia-parra**

---

### 🧰 Tech stack
<p>
  <img alt="Python" src="https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white"/>
  <img alt="OpenCV" src="https://img.shields.io/badge/OpenCV-5C3EE8?logo=opencv&logoColor=black"/>
  <img alt="PLC" src="https://img.shields.io/badge/PLC-Industrial-blue"/>
  <img alt="Docker" src="https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white"/>
  <img alt="Node.js" src="https://img.shields.io/badge/Node.js-339933?logo=node.js&logoColor=white"/>
  <img alt="React" src="https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=black"/>
  <img alt="Odoo" src="https://img.shields.io/badge/Odoo-714B67?logo=odoo&logoColor=white"/>
  <img alt="UI/UX" src="https://img.shields.io/badge/UI%2FUX-Design-black"/>
</p>

---

### 🚀 Proyectos destacados
- **Inspección de objetos (PoC)**  
  Python + OpenCV + **Modbus/OPC UA** → decisión **OK/NG** al PLC.
- **Dashboards de mantenimiento**  
  Trazabilidad, informes automáticos, búsqueda por máquina.
- **Sitios y componentes UI/UX**  
  Diseño accesible, responsive, con performance y escalabilidad.

---
### 📊 Stats (opcionales)
<p>
  <img src="https://github-readme-stats.vercel.app/api?username=cinthia-parra&show_icons=true&hide_title=true&theme=tokyonight&bg_color=0d1117&title_color=58a6ff&text_color=c9d1d9&icon_color=58a6ff&hide_border=true" height="140"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=cinthia-parra&layout=compact&theme=tokyonight&bg_color=0d1117&title_color=58a6ff&text_color=c9d1d9&hide_border=true" height="140"/>
</p>

---

### 🤝 Conecta
- LinkedIn: **https://www.linkedin.com/in/cinthia-parra**
- Correo: **cinthia.parra.solorio@outlook.com**
